...
import { itensListaEstaoReceita } from '@/operacoes/listas'

const props = defineProps({
    ingredientes: Array
})
const receitasEncontradas = ref([])
onMounted(async () => {
    const receitas = await obterReceitas()
    receitasEncontradas.value = receitas.filter((receita) => {
        const possoFazer = itensListaEstaoReceita(receita.ingredientes, props.ingredientes)
        return possoFazer
    })
})
